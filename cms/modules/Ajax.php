...
/* @var $this \cms\Controller object */
/* @var $this \cms\Settings object */
...
class Ajax extends Settings {
    ...
    public function veriSil(){
        ...
        elseif ($table == "ceviri"){
             $al = $this->dbConn->tekSorgu("SELECT * FROM ceviri WHERE id = ".$id);
             $kid = $al["kid"];
             $this->dbConn->sil("DELETE FROM ceviri WHERE id = ".$id);
             $this->ceviriDosyaYaz($kid);
             echo 1;
        }
    ...
    public function removeImage(){
        $id = $_GET["id"];
        $table = $_GET["table"];
        $column = (($table == "dosyalar") ? "dosya" : "resim");
    ...
    public function truncateTable(){
        ...
        if($_SERVER["SERVER_NAME"] == "localhost" or $domain=="det" or $domain == 'vm' or $domain > 0){
            ...
            $disable_table  = array("ayarlar", "boyutlar", "kullanici", "moduller", "sayfakategori","sehirler", "ceviri", "ceviri_kategori");
            $table  = array();
            ...
            $dizinadi='../'.$this->settings->config('folder');
            ...
            {
                if ($isim != "." && $isim != ".." && !in_array( $isim,$disable_folders)){
                    if (is_dir($dizinadi.$isim)){
                        $alt_dizin = opendir($dizinadi.$isim);
                        while(gettype($modul_klasor=readdir($alt_dizin))!="boolean"){
                            if ($modul_klasor != "." && $modul_klasor != ".."){
                                if (is_dir($dizinadi.$isim."/".$modul_klasor)){
                                    $tmp_dizin = opendir($dizinadi.$isim."/temp");
                                    while(gettype($tmp_file=readdir($tmp_dizin))!="boolean") {
                                        if ($tmp_file != "." && $tmp_file != "..") {
                                            unlink($dizinadi.$isim."/".$modul_klasor."/".$tmp_file);
                                        }
                                    }

                                }
                                else {
                                    unlink($dizinadi.$isim."/".$modul_klasor);
                                }
                            }
                        }
                    }
                    else {
                        echo unlink($dizinadi.$isim);
                    }
                }
            }
