...
class Sayfa  extends Settings{
    ...
    public function ekle($id=null)
    {
        ...
        $text.= $form->file(array('url'=>$this->BaseURL('upload')."/".$this->modulName,'folder'=>$this->modulName,'title'=>'Sayfa Resmi','name'=>'SayfaResim','resimBoyut'=>$this->modul_image_size($this->modul_info["id"]),'src'=>((isset($data['tr']['resim'])) ? $data['tr']['resim'] :'')));

        $text.=$form->openBox().$form->openBoxBody();

        $text .= $form->select(array('required'=>true,'title'=>'Sayfa Kategorisi','name'=>'kid','data'=> $form->parent(array('sql'=>"select * from $this->ktable WHERE sil <> 1 ORDER BY sira ASC",'option'=>array('value'=>'id','title'=>'baslik'),'selected'=>((isset($data['tr']['kid'])) ? $data['tr']['kid'] :'')),0,0)));
