    ...
    public function iletisim($id=null)
    {
        ...
        if(is_array($tabsList))
            foreach ($tabsList as $name=>$value):
                ...
                $tabsForm[$name]['text'] = $form->input(array("mask"=>"9 (999) 999 99 99", "icon"=>"mdi mdi-phone-in-talk",'value'=>$this->get_element('telefon_'.$name),'title'=>'Telefon','type'=>'text', 'name'=>'telefon_'.$name,'id'=>'telefon_'.$name));
                ...
                $tabsForm[$name]['text'].= $form->input(array("mask"=>"9 (999) 999 99 99", "icon"=>"mdi mdi-fax",'value'=>$this->get_element('faks_'.$name),'title'=>'Faks ','type'=>'text', 'name'=>'faks_'.$name,'id'=>'faks_'.$name,'help'=>''));
