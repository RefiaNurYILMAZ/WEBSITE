...
class Ikinciel  extends Settings{


    public  $modulName;
    private $ktable = "ikinciel_kategori";
    private $ktablelang = "ikincielkategori_lang";

    /* marka kategori */
    private $mtable = "marka_kategori";
    private $mtablelang = "marka_kategori_lang";
    /* #marka kategori*/
    ...
    public function __construct($settings)
    {
        ...
        $this->tablist = array(
            array("title" => "İkinci El Ürünler", "href" => "liste", "icon" => "mdi mdi-view-sequential"),
            array("title" => "İkinci El Ürün Kategorileri", "href" => "kategoriler", "icon" => "mdi mdi-ungroup"),

            /* marka kategori */
           array("title" => "Markalar", "href" => "kategoriler2", "icon" => "mdi mdi-ungroup"),  //Marka Kategorileri
            /* #marka kategori*/
    ...
    public function liste($id=null)
    {
        ...
        list($sql, $showing, $toplamVeri) = $this->sayfalama(array(
          ...
          "sqlEk"=>"(SELECT baslik FROM $this->ktable WHERE $this->ktable.id = ".$this->table.".kid) as kat_baslik, (SELECT baslik FROM $this->mtable WHERE $this->mtable.id = ".$this->table.".kid2) as marka_baslik",



            /* marka kategori  */
            //"sqlEk2"=>"(SELECT baslik FROM $this->mtable WHERE $this->mtable.id = ".$this->table.".kid2) as marka_baslik"
            /* #marka kategori */
        ...
        $this->SayfaBaslik = "İkinci Ürün Listesi";
        ...
        $text .= $pagelist->PageList(array(
            ...
            /* marka 
            'flitpage' => array("url"=>$filterURL,"title"=>"Marka Kategori Seçiniz",'sql'=>"select * from $this->mtable WHERE sil <> 1 and  ",'option'=>array('value'=>'id','title'=>'baslik'), "name"=>"katFilter", "class"=>"kat_filter", "kat"=>"ustu"),
            /* #marka */


          //  'markaFilter' => array("url"=>$filterURL,"title"=>"Marka Seçiniz",'sql'=>"select * from $this->mtable WHERE sil <> 1  ",'option'=>array('value'=>'id','title'=>'baslik'), "name"=>"markaFilter", "class"=>"marka_baslik"),
            ...
            'p'=>array(
                ...
                /* marka kategori */
                array('dataTitle'=>'marka_baslik', 'class'=>'sort'),
                /* #marka kategori */
            ),
            ...
            'baslik'=>array(
                ...
                /* marka kategori */
                  array('title'=>'Marka Kategori','width'=>'10%'),
                /* #marka kategori*/
                
                array('title'=>'Resimler','width'=>'5%', "align"=>"center"),
    ...
    public function ekle($id=null)
    {
        ...
        foreach ($this->settings->lang('lang') as $dil=>$title):
            ...
            $tabForm[$dil]['text'] .= $form->textEditor(array('value'=>((isset($data[$dil]['ozet']) ? $this->temizle($data[$dil]['ozet']) :'')),'title'=>'Kısa Açıklama','name'=>'ozet','lang'=>$dil,'height' => '350'));
            ...
            $tabForm[$dil]['text'] .= $form->textEditor(array('value'=>((isset($data[$dil]['detay']) ? $this->temizle($data[$dil]['detay']) :'')),'title'=>'Detaylar','name'=>'detay','lang'=>$dil,'height' => '350'));
        ...
        $text.= $form->file(array('url'=>$this->BaseURL('upload')."/".$this->modulName,'folder'=>$this->modulName,'title'=>'Detay Resmi','name'=>'detay_resim','resimBoyut'=>"750x300",'src'=>((isset($data['tr']['detay_resim'])) ? $data['tr']['detay_resim'] :'')));
        ...
        $text .= $form->select(array('required'=>true,'title'=>'Kategori','name'=>'kid','data'=> $form->parent(array('sql'=>"select * from $this->ktable WHERE sil <> 1",'option'=>array('value'=>'id','title'=>'baslik'),'selected'=>((isset($data["tr"]['kid'])) ? $data["tr"]['kid'] :'')),0,0)));

        //$text .= $form->checkbox(array('value'=>((isset($data["tr"]['anasayfa'])) ? $this->temizle($data["tr"]['anasayfa']) :''),'title'=>'Anasayfada Göster','name'=>'anasayfa','help'=>'Anasayfa', "checked"=>((isset($data["tr"]["anasayfa"])) ? "" : "")));

        /* marka KATEGORİ */

         $text .= $form->select(array('title'=>'Marka Kategori','name'=>'kid2','data'=> $form->parent(array('sql'=>"select * from $this->mtable WHERE sil <> 1",'option'=>array('value'=>'id','title'=>'baslik'),'selected'=>((isset($data["tr"]['kid2'])) ? $data["tr"]['kid2'] :'')),0,0)));
        ...
        /* ikinci el */
       // $text .= $form->checkbox(array('value'=>((isset($data["tr"]['ikinciel'])) ? $this->temizle($data["tr"]['ikinciel']) :''),'title'=>'Ikinci El','name'=>'ikinciel','help'=>'Ikinci El Ürün ', "checked"=>((isset($data["tr"]["ikinciel"])) ? "" : "checked")));
        ...
        foreach ($this->settings->lang('lang') as $dil=>$title):
            ...
            if($dil == "tr"):
                $post[$dil] = array(
                    ...
                    //  'resim' => $this->_RESIM_BASE64('resim', $this->modulName,"","jpg","60"),
                    'resim' => $this->_RESIM_BASE64('resim', $this->modulName,"","jpg","60"),
                    'detay_resim' => $this->_RESIM_BASE64('detay_resim', $this->modulName,"","jpg","60"),
                    'aktif'=> ($this->_POST('aktif')) ? $this->_POST('aktif'):0,
                    'kid'=> ($this->_POST('kid')) ? $this->_POST('kid'):0,

                    
                    /* marka KATEGORİ */
                    'kid2'=> ($this->_POST('kid2')) ? $this->_POST('kid2'):0,
                    /* #marka KATEGORİ */


                    /* ikinci el */
                     'ikinciel'=> ($this->_POST('ikinciel')) ? $this->_POST('ikinciel'):0,
                    /* #ikinci el */
    ...
    public function kategoriler($id=null)
    {
        ...
        $this->SayfaBaslik = "İkinci El Kategori Listesi";
    ...
    public function kategoriEkle($id=null)
    {
     ...
     $text .= $form->openColumn(4);
            ...
            $text.= $form->file(array('url'=>$this->BaseURL('upload')."/".$this->ktable,'folder'=>$this->ktable,'title'=>'Sayfa Banner Resmi','name'=>'banner_resim','resimBoyut'=>"375x226",'src'=>((isset($data['tr']['resim'])) ? $data['tr']['resim'] :'')));



            /* ikinci resim alani */
            $text.= $form->file2(array('url'=>$this->BaseURL('upload')."/".$this->ktable,'folder'=>$this->ktable,'title'=>'Anasayfa Icon Resmi','name'=>'banner','resimBoyut'=>"80x80",'src'=>((isset($data['tr']['banner'])) ? $data['tr']['banner'] :'')));
            /* #ikinci resim alani */
    ...
    public function kategoriler2($id=null)
    {
        list($sql, $showing, $toplamVeri) = $this->sayfalama(array(
            "table"=>$this->mtable,
            "kosul"=>(isset($id)) ? "ustu = ".$id : ((!empty($_GET["kelime"])) ? "" : " ustu = 0"),
        ...
        $text .= $pagelist->KategoriList(array(
            ...
            "toplamVeri"=>$toplamVeri,
            "search"=>true,
            'button' => array(array('title'=>'Marka Kategori Ekle','href'=>$this->BaseAdminURL($this->modulName.'/kategoriEkle2'),"class"=>"btn btn-success")),
            'p'=>array(
                array('dataTitle'=>'baslik', 'class'=>'sort'),
            ),

            'tools' =>array(
                array('title'=>'Düzenle','icon'=>'ti-pencil','url'=>$this->BaseAdminURL($this->modulName.'/kategoriEkle2/'),'color'=>'btn-primary'),
                array("disable_delete"=>true,'title'=>'Sil','icon'=>'ti-close','url'=>$this->BaseAdminURL($this->modulName.'/kategoriSil2/'),'color'=>'bg-maroon silButon')
            ),
            'buton'=> array(
                //array("disable_delete"=>true,'type'=>'button2','align'=>"center", 'title'=>'Video Ekle','class'=>'btn bg-olive','dataname'=>'fotoekle','url'=>$this->BaseAdminURL('video/liste/'), "data-icon"=>"mdi mdi-video"),
                //array("disable_delete"=>true,'type'=>'button2','align'=>"center", 'title'=>'3D Modeller','class'=>'btn btn-success','dataname'=>'fotoekle','url'=>$this->BaseAdminURL('Dosyalar/fotoekle/'), "data-icon"=>" ti-envelope", "modul"=>"kategori", "file_type"=>"3d"),
                array('type'=>'radio','dataname'=>'aktif','url'=>$this->BaseAdminURL($this->modulName.'/kategoriDurum2/')),
            ),

            'pdata' => $this->dbConn->sorgu($sql),
            'baslik'=>array(
                array('title'=>'Marka Kategori Adı','width'=>'25%'),
    ...
    public function kategoriEkle2($id=null)
    {
        $this->SayfaBaslik = 'Kategori '.(($id) ? "Düzenle" : "Ekle");
        $text = '';
        $tabForm = array();
        ...
        $text .= $form->formOpen(array('method'=>'POST','action'=> $this->BaseAdminURL($this->modulName.'/kategoriKaydet2'.((isset($id)) ? '/'.$id :'')),'fileUpload'=>true,'id'=>'form_sample_3','class'=>''));
        $text .= $form->openColumn(8);

        if($id) $data = $tabs->tabData($this->mtable,$id);
       ...
       $text .= $form->openColumn(4);
            ...
            $text.= $form->file(array('url'=>$this->BaseURL('upload')."/"."marka_kategori",'folder'=>$this->ktable,'title'=>'Sayfa Banner Resmi','name'=>'banner_resim','resimBoyut'=>"128x60",'src'=>((isset($data['tr']['resim'])) ? $data['tr']['resim'] :'')));
    ...
    public function kategoriKaydet2($id=null)
    {
        ...
        foreach ($this->settings->lang('lang') as $dil=>$title):
            ...
            if($dil == "tr"):
                $post[$dil] = array(
                    ...
                    'aktif'=> ($this->_POST('aktif')) ? $this->_POST('aktif'):0,
                    //'ustu'=>($this->_POST('ustu')) ? $this->_POST('ustu'):0,
                    'resim' => $this->_RESIM_BASE64('banner_resim', $this->mtable),
                    'banner' => $this->_RESIM_BASE64('banner', $this->mtable),
                    'dil' => $dil
                );
            else:
                $post[$dil] = array(
        ...
        if(isset($id) and $id):
            ...
            $this->dbConn->update($this->mtable,$post['tr'],$id);
            foreach ($this->settings->lang('lang') as $dil=>$title):
                if($dil!='tr') {
                    $post[$dil]["url"] = strtolower($this->permalink($post[$dil]["baslik"]))."-".$id;
                    ...
                    if(count($this->dbConn->sorgu("select lang_id from ".$this->ktablelang." where dil='$dil'  and master_id='$id' "))==1)
                        $this->dbConn->update($this->mtablelang, $post[$dil], array('master_id' => $id,'dil'=>$dil));
                    else
                        $this->dbConn->insert($this->mtablelang,array_merge($post[$dil],array('master_id'=>$id)));
                }
            endforeach;
            ...
            $post["tr"]["sira"] = $this->Order($this->ktable);
            $this->dbConn->insert($this->mtable,$post['tr'],$id);
            $lastid = $this->dbConn->lastid();

            $this->dbConn->update($this->mtable, array(
                "url"=>strtolower($this->permalink($post["tr"]["baslik"]))."-".$lastid
            ),$lastid);
            ...
            foreach ($this->settings->lang('lang') as $dil=>$title):
                ...
                if($dil!='tr') $this->dbConn->insert($this->mtablelang,array_merge($post[$dil],array('dil'=>$dil,'master_id'=>$lastid)));

            endforeach;
        endif;
        $this->RedirectURL($this->BaseAdminURL($this->modulName.'/kategoriler2'.(($this->_POST('ustu')) ? "/".$this->_POST('ustu'):'')));
    }
    ...
    public function kategoriSil2($id=null)
    {
        if ($id){
            $date = date("Y-m-d H:i:s");
            $this->dbConn->update($this->mtable, array("sil"=>1, "silme_tarihi"=>$date, "silen"=>$this->getUserInfo("adi")), array("id"=>$id));
            $this->dbConn->langUpdate($this->mtablelang, array("sil"=>1, "silme_tarihi"=>$date, "silen"=>$this->getUserInfo("adi")), array("master_id"=>$id));
        }
        $this->RedirectURL($this->BaseAdminURL($this->modulName.'/kategoriler2'));
    }




    public function kategoriDurum2($id=null)
    {
        $durum = ((isset($_GET['durum'])) ? $_GET['durum'] : null);
        $id = ((isset($_GET['id'])) ? $_GET['id'] : null);
        $tr_duzenle = $this->dbConn->update($this->mtable,array('aktif'=>$durum),$id);
        $lang_duzenle = $this->dbConn->langUpdate($this->mtablelang,array('aktif'=>$durum),$id);
