...
class Katalog  extends Settings{
    ...
    public function __construct($settings)
    {
        ...
        $this->tablist = array(
            array("title" => "Döküman Listesi", "href" => "liste", "icon" => "mdi mdi-view-sequential"),
    ...
    public function liste($id=null)
    {
        ...
        $text .= $pagelist->PageList(array(
            'flitpage' => array("url"=>$filterURL,"title"=>"Kategori Seçiniz",'sql'=>"select * from proje_kategori WHERE sil <> 1 ",'option'=>array('value'=>'id','title'=>'baslik'), "name"=>"kategoriFilter", "class"=>"flit_filter" ),
            'id'=>$id,
            ...
            "resim"=>true,
            "search"=>true,
            'button' => array(array('title'=>'Döküman Ekle','href'=>$this->BaseAdminURL($this->modulName.'/ekle'),"class"=>"btn btn-success")),
            'p'=>array(
    ...
    {
        $this->SayfaBaslik = 'Katalog '.(($id) ? "Düzenle" : "Ekle");
        $text = '';
        $tabForm = array();
        ...
        $fileUpload = $modal->fileLoad(['name'=>'files','params' => array('modul'=>$this->modulName,'folder'=>'../'.$this->settings->config('folder').$this->modulName.'/','baslik'=>'baslik_tr','id'=>$id),'sql'=>(($id) ? $this->dbConn->sorgu("select * from dosyalar where type='{$this->modulName}' and data_id='$id' and tur = 'dosya' ORDER   BY  sira"):array())]);
        $text .= $modal->infoform(array('title'=>'Pdf Dosya Yükle','govde'=>$fileUpload));*/
        ...
        $text .= $modal->infoform(array('title'=>'Dosya Yükle','govde'=>$fileUpload));
        ...
        $text .= $form->select(array('required'=>true,'title'=>'Kategori','name'=>'kid','data'=> $form->parent(array('sql'=>"select * from proje_kategori WHERE sil <> 1",'option'=>array('value'=>'id','title'=>'baslik'),'selected'=>((isset($data["tr"]['kid'])) ? $data["tr"]['kid'] :'')),0,0)));
