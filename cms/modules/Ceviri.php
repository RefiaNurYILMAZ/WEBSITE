...
class Ceviri  extends Settings{
    ...
    public function liste($id=null)
    {
        ...
        $p_list = array(
            ...
            array('dataTitle'=>'kid', 'type'=>'short_text'),
        );
    ...
    public function kaydet($id=null)
    {
        ...
        if(isset($id) and $id){
            ...
            $al = $this->dbConn->tekSorgu("SELECT * FROM $this->table WHERE `key` = '".$this->_POST('key')."' and kid = '".$this->_POST('kid')."'");

            if (is_array($al)){
                //KEY DAHA ÖNCE EKLENMİŞSE SİL
                $this->dbConn->sil("DELETE FROM $this->table WHERE `key` = '".$this->_POST('key')."' and kid = '".$this->_POST('kid')."'");
            }
