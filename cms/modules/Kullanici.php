    ...
    public function ekle($id=null)
    {
        ...
        if ($user_type == 1){
            ...
            if ($this->user_id != $id){
                ...
                $text .= $form->selectmulti(array('required'=>true,'title'=>'Yetkiler','name'=>'yetkiler','data'=> $form->parent(array('sql'=>"select * from moduller WHERE aktif = 1 and id IN ($yetki_list)",'option'=>array('value'=>'id','title'=>'baslik'),'selected'=>((isset($data['yetkiler'])) ? $data['yetkiler'] :'')),0,0)));
